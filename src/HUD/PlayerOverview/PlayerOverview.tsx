...
const PlayerOverview = ({ player, show, veto, players, round }: IProps) => {
  ...
  const getData = () => {
    ...
    const data = {
      ...
      hsp:
        overall.kills !== 0
          ? ((100 * overall.killshs) / overall.kills).toFixed(0)
          : "0",
    };
    return data;
  };
  ...
  let side = "";
  const inGamePlayer = players.find(
    (inGamePlayer) => inGamePlayer.steamid === player.steamid
  );
  ...
  return (
    <div className={`player-overview ${show ? "show" : ""} ${side}`}>
      ...
      <div className="player-overview-username">
        {url && countryName ? (
          <img
      ...
      <div className="player-overview-stats">
        <div className="player-overview-stat">
          ...
          <div className="panel">
            <div
              className="filling"
              style={{
                width: `${calcWidth(data.kills, data.kills <= 30 ? 30 : 40)}%`,
              }}
            ></div>
          </div>
        </div>
        <div className="player-overview-stat">
          ...
          <div className="panel">
            <div
              className="filling"
              style={{ width: `${calcWidth(data.hsp, 100)}%` }}
            ></div>
          </div>
        </div>
        <div className="player-overview-stat">
          ...
          <div className="panel">
            <div
              className="filling"
              style={{ width: `${calcWidth(data.adr)}%` }}
            ></div>
          </div>
        </div>
        <div className="player-overview-stat">
          ...
          <div className="panel">
            <div
              className="filling"
              style={{ width: `${calcWidth(Number(data.kpr) * 100)}%` }}
            ></div>
          </div>
        </div>
      </div>
