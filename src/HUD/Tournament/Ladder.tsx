...
const joinParents = (
  ...
  const parents = matchups.filter(
    (m) => m.winner_to === matchup._id || m.loser_to === matchup._id
  );
  if (!parents.length) return matchup;
  matchup.parents.push(
    ...parents.map((parent) => joinParents(parent, matchups))
  );
...
const copyMatchups = (
  currentMatchups: I.TournamentMatchup[]
): I.DepthTournamentMatchup[] => {
  const matchups = JSON.parse(
    JSON.stringify(currentMatchups)
  ) as I.DepthTournamentMatchup[];
  return matchups;
};

const setDepth = (
  matchups: I.DepthTournamentMatchup[],
  matchup: I.DepthTournamentMatchup,
  depth: number,
  force = false
) => {
  const getParents = (matchup: I.DepthTournamentMatchup) => {
    return matchups.filter(
      (parent) =>
        parent.loser_to === matchup._id || parent.winner_to === matchup._id
    );
  };

  if (!matchup.depth || force) {
    matchup.depth = depth;
    getParents(matchup).forEach((matchup) =>
      setDepth(matchups, matchup, depth + 1)
    );
  }
...
const Ladder = ({ tournament, matches, teams }: Props) => {
  const renderBracket = (
    ...
    const parentsToRender = matchup.parents.filter(
      (matchupParent) => matchupParent.loser_to !== matchup._id
    );
        ...
        <div className="bracket-details">
          <div
            className={`match-connector ${
              !matchup.parents.length || parentsToRender.length === 0
                ? "first-match"
                : ""
            } ${isLast ? "last-match" : ""}`}
          ></div>
          {parentsToRender.length === 1 ? (
            <div className="loser-parent-indicator"></div>
          ) : null}
          <div className={`match-details ${isCurrent ? "current" : ""}`}>
            <div className="team-data">
              <div className="team-logo">
                {match.left.logo ? (
                  <img
                    src={`${apiUrl}/api/teams/logo/direct/${match.left.logo}`}
                    alt="Logo"
                  />
                ) : null}
              </div>
            ...
            <div className="team-data">
              <div className="team-logo">
                {match.right.logo ? (
                  <img
                    src={`${apiUrl}/api/teams/logo/direct/${match.right.logo}`}
                    alt="Logo"
                  />
                ) : null}
              </div>
  ...
  const matchupWithDepth = setDepth(
    matchups,
    joinedParents as I.DepthTournamentMatchup,
    0
  );
